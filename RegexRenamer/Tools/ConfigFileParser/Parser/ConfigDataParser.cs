...
namespace ConfigFileParser.Parser
{
	/// <summary>
	/// 	Responsible for parsing an string from an ini file, and creating
	/// 	an <see cref="ConfigData"/> structure.
	/// </summary>
    public class ConfigDataParser
    {
        ...
        /// </param>
        public ConfigDataParser(IniParserConfiguration parserConfiguration)
        {
        ...
        /// </param>
        /// <returns>
        ///     An <see cref="ConfigData"/> instance with the data contained in
        ///     the <paramref name="iniDataString"/> correctly parsed an structured.
        /// </returns>
        /// <exception cref="ParsingException">
        ///     Thrown if the data could not be parsed
        /// </exception>
        public ConfigData Parse(string iniDataString)
        {
            
            ConfigData iniData = Configuration.CaseInsensitive ? new ConfigDataCaseInsensitive() : new ConfigData();
            ...
            try
            {
                ...
                for (int lineNumber = 0; lineNumber < lines.Length; lineNumber++)
                {
                    ...
                    {
                        var errorEx = new InvalidOperationException($"{ex.Message} ({lineNumber+1}) ({line})", ex);
            ...
            return (ConfigData)iniData.Clone();
        }
        #endregion
        ...
        protected virtual void ProcessLine(string currentLine, ConfigData currentIniData)
        {
        ...
        /// </param>
        protected virtual void ProcessSection(string line, ConfigData currentIniData)
        {
            ...
            if (currentIniData.Sections.ContainsSection(sectionName))
            {
                ...
                throw new InvalidOperationException(string.Format("Duplicate section with name '{0}' on line '{1}'", sectionName, line));
            }
        ...
        ///     The string to be processed
        /// </param>
        protected virtual void ProcessKeyValuePair(string line, ConfigData currentIniData)
        {
            ...
            if (string.IsNullOrEmpty(_currentSectionNameTemp))
            {
                ...
                {
                    throw new InvalidOperationException("key value pairs must be enclosed in a section");
                }
        ...
        protected virtual void HandleDuplicatedKeyInCollection(string key, string value, KeyDataCollection keyDataCollection, string sectionName)
        {
            ...
            {
                throw new InvalidOperationException(string.Format("Duplicated key '{0}' found in section '{1}", key, sectionName));
            }
