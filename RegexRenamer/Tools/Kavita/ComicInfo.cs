...
public static void CleanComicInfo(ComicInfo? info)
    {
        ...
        info.Writer = Parser.CleanAuthor(info.Writer);
        info.Colorist = Parser.CleanAuthor(info.Colorist);
        info.Editor = Parser.CleanAuthor(info.Editor);
        info.Inker = Parser.CleanAuthor(info.Inker);
        info.Letterer = Parser.CleanAuthor(info.Letterer);
        info.Penciller = Parser.CleanAuthor(info.Penciller);
        info.Publisher = Parser.CleanAuthor(info.Publisher);
        info.Imprint = Parser.CleanAuthor(info.Imprint);
        info.Characters = Parser.CleanAuthor(info.Characters);
        info.Translator = Parser.CleanAuthor(info.Translator);
        info.CoverArtist = Parser.CleanAuthor(info.CoverArtist);
        info.Teams = Parser.CleanAuthor(info.Teams);
        info.Locations = Parser.CleanAuthor(info.Locations);
    }
