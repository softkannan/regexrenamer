...
public interface IDirectoryService
{
    ...
    bool CheckWriteAccess(string directoryName);
...
public class DirectoryService : IDirectoryService
{
    ...
    private static readonly Regex FileCopyAppend = new Regex(@"\(\d+\)",
        MatchOptions,
        Parser.RegexTimeout);
    public static readonly string BackupDirectory = Path.Combine(Directory.GetCurrentDirectory(), "config", "backups");
        ...
        SearchOption searchOption = SearchOption.TopDirectoryOnly)
    {
        ...
        return Directory.EnumerateFiles(path, "*", searchOption)
            .Where(file =>
                reSearchPattern.IsMatch(Path.GetExtension(file)) && !Path.GetFileName(file).StartsWith(Parser.MacOsMetadataFileStartsWith));
    }
            ...
            return Directory.EnumerateFiles(path, "*", searchOption)
                .Where(file =>
                {
                    ...
                    return reSearchPattern.IsMatch(fileName) &&
                           !fileName.StartsWith(Parser.MacOsMetadataFileStartsWith);
                });
        }

        return Directory.EnumerateFiles(path, "*", searchOption).Where(file =>
            !Path.GetFileName(file).StartsWith(Parser.MacOsMetadataFileStartsWith));
    }
    ...
    public void CopyFileToDirectory(string fullFilePath, string targetDirectory)
    {
        ...
        {
            _logger.Error(ex, "There was a critical error when copying {File} to {Directory}", fullFilePath, targetDirectory);
        }
    }
    ...
    public void ClearDirectory(string directoryPath)
    {
        ...
        {
            _logger.Error(ex, "[ClearDirectory] Could not delete {DirectoryPath} due to permission issue", directoryPath);
        }
    ...
    public bool CopyFilesToDirectory(IEnumerable<string> filePaths, string directoryPath, string prepend = "")
    {
        ...
        try
        {
            foreach (var file in filePaths)
            {
                ...
                var targetFile = new FileInfo(RenameFileForCopy(file, directoryPath, prepend));

                fileInfo.CopyTo(Path.Combine(directoryPath, targetFile.Name));
            }
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "Unable to copy {File} to {DirectoryPath}", currentFile, directoryPath);
            return false;
        }
    ...
    public bool CopyFilesToDirectory(IEnumerable<string> filePaths, string directoryPath, IList<string> newFilenames)
    {
        ...
        try
        {
            foreach (var file in filePaths)
            {
                ...
                var fileInfo = new FileInfo(file);
                var targetFile = new FileInfo(RenameFileForCopy(newFilenames[index] + fileInfo.Extension, directoryPath));
    ...
    private string RenameFileForCopy(string fileToCopy, string directoryPath, string prepend = "")
    {
        while (true)
        {
            ...
            var targetFile = new FileInfo(Path.Combine(directoryPath, filename));
            if (!targetFile.Exists)
            {
                return targetFile.FullName;
            }

            var noExtension = Path.GetFileNameWithoutExtension(fileInfo.Name);
            ...
            var newFilename = prepend + noExtension + Path.GetExtension(fileInfo.Name);
            fileToCopy = Path.Combine(directoryPath, newFilename);
        }
    }
    ...
    public Dictionary<string, string> FindHighestDirectoriesFromFiles(IEnumerable<string> libraryFolders, IList<string> filePaths)
    {
        ...
        foreach (var folder in libraryFolders.Select(Parser.NormalizePath))
        {
            ...
            foreach (var file in filePaths.Select(Parser.NormalizePath))
            {
                ...
                var fullPath = Parser.NormalizePath(Path.Combine(folder, parts[parts.Count - 1]));
    ...
    public IEnumerable<string> GetDirectories(string folderPath, GlobMatcher matcher)
    {
        ...
        return GetDirectories(folderPath)
            .Where(folder => !matcher.ExcludeMatches(
                $"{ new DirectoryInfo(folder).Name}{Path.AltDirectorySeparatorChar}"));
    }
    ...
    {
        _logger.Debug("[ScanFiles] called on {Path}", folderPath);
        var files = new List<string>();
        if (!Exists(folderPath)) return files;

        var potentialIgnoreFile = Path.Combine(folderPath, KavitaIgnoreFile);
        if (matcher == null)
        {
        ...
        else
        {
            var foundFiles = GetFilesWithCertainExtensions(folderPath,
                    fileTypes)
                .Where(file => !matcher.ExcludeMatches(new FileInfo(file).Name));
    ...
    // <returns></returns>
    public bool CheckWriteAccess(string directoryName)
    {
        try
        {
            ExistOrCreate(directoryName);
            File.WriteAllText(
                Path.Combine(directoryName, "test.txt"),
                string.Empty);
        }
        catch (Exception)
        {
    ...
    private static void FlattenDirectory(DirectoryInfo root, DirectoryInfo directory, ref int directoryIndex)
    {
        if (!root.FullName.Equals(directory.FullName))
        {
            ...
            foreach (var file in directory.EnumerateFiles().OrderBy(file => file.FullName))
            {
        ...
        foreach (var subDirectory in directory.EnumerateDirectories().OrderBy(d => d.FullName))
        {
