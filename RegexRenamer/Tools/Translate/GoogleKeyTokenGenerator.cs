...
public class GoogleKeyTokenGenerator
{
    ...
    protected virtual async Task<ExternalKey> GetNewExternalKeyAsync()
    {
        ...
        try
        {
            ...
            //Regex RE_TKK = new Regex("tkk:\'(.+?)\'", RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.CultureInvariant);
            //Regex RE_RAWTKK = new Regex("tkk:\'(.+?)\'", RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.CultureInvariant);

            var tkkText = result.GetTextBetween(@"tkk:'", "',");

            if (tkkText == null)
                throw new InvalidOperationException("Unknown TKK position");

            var splitted = tkkText.Split('.');
            if (splitted.Length != 2 || !long.TryParse(splitted[1], out tkk))
                throw new InvalidOperationException($"Unknown TKK format. TKK: {tkkText}");
