...
namespace EpubSharp
{
    ...
    public class EpubWriter
    {
        ...
        public void ClearSeriesAndVolume()
        {
            foreach (var meta in format.Opf.Metadata.Metas.Where(e => e.Name == "calibre:series" ||
                  e.Name == "calibre:series_index").ToList())
            {
                format.Opf.Metadata.Metas.Remove(meta);
            }
        }

        public void SetSeries(string series, int volume = 0)
        {
            if (string.IsNullOrWhiteSpace(series)) throw new ArgumentNullException(nameof(series));

            // Remove existing series and volume
            foreach (var meta in format.Opf.Metadata.Metas.Where(e => e.Name == "calibre:series" ||
                  e.Name == "calibre:series_index").ToList())
            {
                format.Opf.Metadata.Metas.Remove(meta);
